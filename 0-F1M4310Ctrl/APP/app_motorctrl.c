...
void Get_MotorData(uint8_t Data[],MotorData *motor)
{
	...
	motor->rxpara.rPOS = uint_to_float1(temp,-(motor->PMAX),motor->PMAX,16);//printf("%.2f\r\n",motor->rxpara.rPOS);ERROR1
	temp = 0;
...
//	printf("motorfeedback:%.2f,%.2f,%.2f\r\n",motor->rxpara.rPOS,motor->rxpara.rVEL,motor->rxpara.rT);
}
...
void Sent_MotorData(MotorData* motor)
{
	...
	temp = float_to_uint1(motor->kp,0,motor->KP_MAX,12);//ERROR3
